...
public class World extends Region {
	...
	private static final CssMetaData<World, Color> BACKGROUND_COLOR = FACTORY
			.createColorCssMetaData("-background-color", s -> s.backgroundColor, Color.web("#000066"), false);
	private final StyleableProperty<Color> backgroundColor;
	private static final CssMetaData<World, Color> FILL_COLOR = FACTORY.createColorCssMetaData("-fill-color",
			s -> s.fillColor, Color.web("#FFFFFF"), false);
	private final StyleableProperty<Color> fillColor;
	private static final CssMetaData<World, Color> STROKE_COLOR = FACTORY.createColorCssMetaData("-stroke-color",
			s -> s.strokeColor, Color.BLACK, false);
	private final StyleableProperty<Color> strokeColor;
	private static final CssMetaData<World, Color> HOVER_COLOR = FACTORY.createColorCssMetaData("-hover-color",
			s -> s.hoverColor, Color.web("#d9f2e5"), false);
	private final StyleableProperty<Color> hoverColor;
	private static final CssMetaData<World, Color> PRESSED_COLOR = FACTORY.createColorCssMetaData("-pressed-color",
			s -> s.pressedColor, Color.web("#ff9900"), false);
	private final StyleableProperty<Color> pressedColor;
	private static final CssMetaData<World, Color> SELECTED_COLOR = FACTORY.createColorCssMetaData("-selected-color",
			s -> s.selectedColor, Color.web("#ff9900"), false);
	...
	private void handleMouseEvent(final MouseEvent event, final EventHandler<MouseEvent> eventHandler) {
		...
		if (MOUSE_ENTERED == eventType) {
			...
			if (isHoverEnabled()) {
				...
				for (SVGPath path : paths) {
					path.setFill(
							null == country.getColor() || country == getSelectedCountry() ? color : country.getColor());
				}
			}
		}
	...
	private Properties readProperties(final String svgPropertiesFilename) {
		...
		try (InputStream resourceStream = loader.getResourceAsStream(svgPropertiesFilename)) {
			...
			System.out.println("Properties file read problem!");
		}
